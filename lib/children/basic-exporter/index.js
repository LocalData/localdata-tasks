...
// entry.
function getMetadata(surveyId) {
  return Response.collection.groupAsync(
    {},
...
async.series([
  ...
  function (next) {
    Promise.join(
      getMetadata(surveyId),
      forms.getFieldSorter(surveyId),
      function (metadata, orderFields) {
        infoFields = metadata[0];
        // Order response fields according to the original survey form question
        // order.
        responseFields = orderFields(metadata[1]);
