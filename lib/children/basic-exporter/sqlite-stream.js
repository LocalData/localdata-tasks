...
function SQLiteStream(options) {
  ...
  this.getData = function getData(item) {
    ...
    var ret = {
      ...
      $photos: item.properties.files[0] || ''
    };

    var i;
    if (fileCount > 1) {
      for (i = 1; i < fileCount; i += 1) {
        ret['$photos' + i] = item.properties.files[i];
      }
    }
    ...
    for (i = 0, len = infoFields.length; i < len; i += 1) {
      field = infoFields[i];
      ret[itemFieldMod(field)] = item.properties.info[field];
    }
    for (i = 0, len = responseFields.length; i < len; i += 1) {
      field = responseFields[i];
      ret[responseFieldMod(field)] = item.properties.responses[field];
    }
